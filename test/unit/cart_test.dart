...
void main() {
  ...
  group('option validation', () {
    test(
      'a product with variants cannot be added without choosing one',
      () async {
    ...
    test('a sold-out product without variants is refused', () async {
      ...
      final result = container
          ...
          .add(Fixtures.soldOut);

      expect(result.error, CartAddError.outOfStock);
    });

    test(
      'adding beyond available stock is refused and reports the ceiling',
      () async {
        final container = await makeContainer();
        final cart = container.read(cartProvider.notifier);
        final variant = Fixtures.pyjamaWithSizes.findVariant(
        ...
        expect(
          cart.add(Fixtures.pyjamaWithSizes, variant: variant).isSuccess,
          isTrue,
        );
        expect(
          cart.add(Fixtures.pyjamaWithSizes, variant: variant).isSuccess,
  ...
  group('totals', () {
    ...
    test('the variant price wins over the product price', () async {
      ...
      container
          .read(cartProvider.notifier)
          .add(product, variant: product.variants.first);
  ...
  group('persistence', () {
    ...
    test(
      'corrupt stored data yields an empty cart rather than a crash',
      () async {
        final container = await makeContainer(
